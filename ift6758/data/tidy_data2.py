...
def tidy(df) -> pd.DataFrame:
    ...
    for i in range(df.shape[1]):
        ...
        for j in range(len(allplays_data)):
            ...
            time_1 = int(allplays_data[j]["about"]["periodTime"][0:2]) * 60 + int(
                allplays_data[j]["about"]["periodTime"][3:5]
            )
            time_period_1 = (allplays_data[j]["about"]["period"] - 1) * 1200

            if j > 0:
                time_2 = int(
                    allplays_data[j - 1]["about"]["periodTime"][0:2]
                ) * 60 + int(allplays_data[j - 1]["about"]["periodTime"][3:5])
            ...
            if (
                allplays_data[j]["result"]["eventTypeId"] == "SHOT"
                or allplays_data[j]["result"]["eventTypeId"] == "GOAL"
            ):
                ...
                team_away_name.append(
                    df.iloc[:, i]["gameData"]["teams"]["away"]["name"]
                )
                team_home_name.append(
                    df.iloc[:, i]["gameData"]["teams"]["home"]["name"]
                )
                team_name.append(allplays_data[j]["team"]["name"])
                is_goal.append(allplays_data[j]["result"]["eventTypeId"] == "GOAL")
                coordinate_x.append(
                    allplays_data[j]["coordinates"]["x"]
                    if "x" in allplays_data[j]["coordinates"]
                    else np.nan
                )
                coordinate_y.append(
                    allplays_data[j]["coordinates"]["y"]
                    if "y" in allplays_data[j]["coordinates"]
                    else np.nan
                )
                shot_type.append(
                    allplays_data[j]["result"]["secondaryType"]
                    if "secondaryType" in allplays_data[j]["result"]
                    else np.nan
                )
                strength.append(
                    allplays_data[j]["result"]["strength"]["name"]
                    if allplays_data[j]["result"]["eventTypeId"] == "GOAL"
                    else np.nan
                )
                if (
                    allplays_data[j]["players"][z]["playerType"] == "Shooter"
                    or allplays_data[j]["players"][z]["playerType"] == "Scorer"
                    for z in range(len(allplays_data[j]["players"]))
                ):
                    shooter_name.append(
                        [
                            allplays_data[j]["players"][z]["player"]["fullName"]
                            for z in range(len(allplays_data[j]["players"]))
                        ][0]
                    )
                if (
                    allplays_data[j]["players"][z]["playerType"] == "Goalie"
                    for z in range(len(allplays_data[j]["players"]))
                ):
                    goalie_name.append(
                        [
                ...
                empty_net.append(
                    True
                    if "emptyNet" in allplays_data[j]["result"]
                    and allplays_data[j]["result"]["emptyNet"] == True
                    else False
                )
                ...
                if j > 0:
                    ...
                    last_coord_x.append(
                        allplays_data[j - 1]["coordinates"]["x"]
                        if "x" in allplays_data[j - 1]["coordinates"]
                        else np.nan
                    )
                    last_coord_y.append(
                        allplays_data[j - 1]["coordinates"]["y"]
                        if "y" in allplays_data[j - 1]["coordinates"]
                        else np.nan
                    )
                    rebound.append(
                        allplays_data[j - 1]["result"]["eventTypeId"] == "SHOT"
                    )
    ...
    df_main["from_last_distance"] = np.sqrt(
        (df_main["coordinate_x"] - df_main["last_coord_x"]) ** 2
        + (df_main["coordinate_y"] - df_main["last_coord_y"]) ** 2
    )

    df_main["angle"] = angle_between(df_main["coordinate_x"], df_main["coordinate_y"])
    df_main["last_angle"] = angle_between(
        df_main["last_coord_x"], df_main["last_coord_y"]
    )
