...
def tidy(df) -> pd.DataFrame:
    ...
    for i in df:
        allplays_data = df[:,i]['liveData']['plays']['allplays']
        for j in range(len(allplays_data)):
            if(j['result']['eventTypeId'] == "SHOT" or "GOAL"):
                ...
                if allplays_data[j]['players']['playerType'] == "Shooter" or 'Scorer':
                    shooter_name.append(allplays_data[j]['players']['fullName'])
                elif allplays_data[j]['players']['playerType']=="Goalie":
                    goalie_name.append(allplays_data[j]['players']['fullName'])
                empty_net.append(True if 'emptyNet' in allplays_data[j]['result'] and allplays_data[j]['result']['emptyNet']==True else False)

    
    assert(all(len(lists) == len(game_id) for lists in [event_idx, period_time, period, game_id, team_away_name, team_home_name, is_goal, coordinate_x,\
     coordinate_y, shot_type, strength, shooter_name, goalie_name, empty_net, team_name]) )

    df2 = pd.DataFrame(np.column_stack([event_idx, period_time, period, game_id, team_away_name, team_home_name, is_goal, coordinate_x,\
     coordinate_y, shot_type, strength, shooter_name, goalie_name, empty_net, team_name]),
